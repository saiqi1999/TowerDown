 ...
 *
 * Ownership boundary:
 * 本文件拥有基地就绪订阅、视觉状态通知、单例面板和输入阻断状态。
 *
...
export class BaseInteractionController {
    ...
    private setBaseVisualState: ((state: 'idle' | 'ready') => void) | null = null;
    ...
    public setup(config: {
        ...
        this.setBaseVisualState = config.setBaseVisualState;
    ...
    public destroy(): void {
        ...
        this.panel?.dispose();
        this.panel = null;
        this.setBaseVisualState = null;
        this.onOpenStateChanged = null;
    }

    private onCounterChanged(snapshot: EnemyKillSnapshot): void {
        this.ready = snapshot.ready;
        this.setBaseVisualState?.(this.ready ? 'ready' : 'idle');
        if (!this.ready && this.openState) this.close();
    }
}
